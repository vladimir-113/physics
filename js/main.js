...
function setupControls() {
  ...
  // Переключатель До/После взрыва.
  $('modeBeforeBtn').addEventListener('click', () => setMapMode('before'));
  $('modeAfterBtn').addEventListener('click', () => setMapMode('after'));
...
function detonateNow() {
  ...
  // Переключаем карту в режим "после взрыва" — город разрушен, пожары, кратер.
  setMapMode('after');

  // Сначала показываем взрыв на canvas...
  switchTab('sim');
  ...
  // ...а через 5 секунд переключаем на карту чтобы было видно как изменился город.
  setTimeout(() => {
    switchTab('map');
    showHint('Город после взрыва — кратер, пожары, разрушения');
  }, 5500);
