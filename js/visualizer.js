...
function drawExplosion(w, h, dt, now) {
  ...
  // Завершение анимации (для показа итогов).
  if (elapsed > 6 && state.detonating && onComplete) {
    onComplete();
    state.detonating = false; // не вызывать дважды, но сохраняем нарисованное состояние
    state.detonating = true;  // оставляем гриб на экране
  }
