...
export function initVisualizer(canvasId, callbacks = {}) {
  ...
  if (!canvas) {
    console.error('[viz] Canvas #' + canvasId + ' not found in DOM');
    return;
  }
  ctx = canvas.getContext('2d');
  if (!ctx) {
    console.error('[viz] Failed to get 2D context');
    return;
  }
  onTimerUpdate = callbacks.onTimerUpdate || null;
  onComplete = callbacks.onComplete || null;

  // Фоллбэк: 1×1 чтобы canvas был валидным с самого начала.
  ...
  // Наблюдаем за изменениями размера .viz-area — ловим момент когда вкладка станет видимой.
  ...
  console.log('[viz] initialized, canvas size:', canvas.width, 'x', canvas.height);
}
...
export function resizeCanvas() {
  ...
  if (canvas.width === newW && canvas.height === newH) return; // не сбрасываем зря
  canvas.width = newW;
  canvas.height = newH;
...
function draw(now) {
  ...
  try {
    ...
    // Если что-то упало — логируем, но не валим цикл.
    if (frameCounter % 60 === 0) console.error('[viz] draw error:', err);
  }
...
function drawIdle(w, h, now) {
  ...
  // Концентрические пульсирующие кольца — сразу видно что цикл живой.
  for (let i = 0; i < 3; i++) {
  ...
  ctx.fillStyle = `rgba(255, 87, 34, ${0.6 + pulse * 0.4})`;
  ctx.beginPath();
  ...
  ctx.fillText('ОЖИДАНИЕ ДЕТОНАЦИИ', cx, cy - 60);
